...
void ssd1306_draw_square(ssd1306_t *ssd, uint8_t top, uint8_t left, bool value, uint8_t size, bool fill)
{
    // Desenha bordas horizontais
    for (uint8_t x = left; x < left + size; ++x)
    {
        ssd1306_pixel(ssd, x, top, value);            // Borda superior
        ssd1306_pixel(ssd, x, top + size - 1, value); // Borda inferior
    }

    // Desenha bordas verticais
    for (uint8_t y = top; y < top + size; ++y)
    {
        ssd1306_pixel(ssd, left, y, value);            // Borda esquerda
        ssd1306_pixel(ssd, left + size - 1, y, value); // Borda direita
    }

    // Preenche o interior, se necessário
    if (fill && size > 2)
    {
        for (uint8_t x = left + 1; x < left + size - 1; ++x)
        {
            for (uint8_t y = top + 1; y < top + size - 1; ++y)
            {
                ssd1306_pixel(ssd, x, y, value);
            }
        }
